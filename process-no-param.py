...
def processRawFile(file, nbCoeff, data_curves):
  data = np.genfromtxt( file , usecols =(5, 6), delimiter=";", skip_header=2, encoding="latin1", dtype=None)
  ...
  for i, el in enumerate(data):
    ...
    chargeN.append(float(el[1].replace(',','.')))
  ...
  PMin = min(chargeN[borne2:min(2*borne2, len(chargeN))])
  PMaxFil = max(chargeN[borne2:len(chargeN)])
  for i in range(incr, len(lissage)):
    if lissage[i] < PMax/2:
      d3 = depla[i]
      break
  ...
  fout = open(os.path.splitext(file)[0] + ".csv.out", 'w')
  ...
  for i in range(len(aire)):
    fout.write('{};{};{};{};{}\n'.format(depla[i], chargeN[i], lissage[i], PPmax[i], aire[i]))
  fout.close()
